...
class AddNewFloorPage extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      data: [],
  ...
  redirectToListPage = () => {
    this.props.history.push('/manage-floor-list')
  }
  ...
  handleSubmit = e => {
    ...
    this.props.attributeFloorAddRequest(data)
  }
  ...
  render() {
    return (
      <Container fluid className="add-new-attribute">
        <Helmet>
          <title>{I18nUtils.t('att-floor-add-page-title')}</title>
        </Helmet>
        <div className="page-title">
          <h1>
            <i className="fa fa-signal" aria-hidden="true" />
            {I18nUtils.t('att-floor-add-page-title')}
          </h1>
        </div>
        <ValidationForm
...
AddNewFloorPage.propTypes = {
  history: PropTypes.object,
  attributeFloorAddRequest: PropTypes.func,
...
const mapDispatchToProps = dispatch => ({
  attributeFloorAddRequest: data =>
    dispatch(AttributeActions.attributeFloorAddRequest(data))
})

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(AddNewFloorPage))
