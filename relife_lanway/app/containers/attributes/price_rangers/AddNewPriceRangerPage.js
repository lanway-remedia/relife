...
class AddNewPriceRangerPage extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      data: [],
  ...
  handleSubmit = e => {
    ...
    this.props.attributePriceAddRequest(data)
  }
  ...
  render() {
    return (
      <Container fluid className="add-new-attribute">
        <Helmet>
          <title>{I18nUtils.t('att-price-add-page-title')}</title>
        </Helmet>
        <div className="page-title">
          <h1>
            <i className="fa fa-signal" aria-hidden="true" />
            {I18nUtils.t('att-price-add-page-title')}
          </h1>
        </div>
        <ValidationForm
...
AddNewPriceRangerPage.propTypes = {
  history: PropTypes.object,
  attributePriceAddRequest: PropTypes.func,
...
const mapDispatchToProps = dispatch => ({
  attributePriceAddRequest: data =>
    dispatch(AttributeActions.attributePriceAddRequest(data))
})

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(AddNewPriceRangerPage))
