...
class ManagePriceRangerListPage extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      count: 0,
  ...
  componentDidMount() {
    this.getAttributePriceRangerList()
  }
  ...
  redirectToAddNew = () => {
    this.props.history.push('/add-new-price-ranger')
  }

  redirectToEdit = att => {
    this.props.history.push(`/edit-price-ranger/${att.id}`)
  }
  ...
  okFunction = att => {
    ...
    this.props.attributePriceDeleteRequest(att.id)
    this.setState({
      title: att.title
    })
  ...
  render() {
    ...
    return (
      <Container fluid className="manage-attribute-list">
        <Helmet>
          <title>{I18nUtils.t('att-price-page-title')}</title>
        </Helmet>
        <div className="page-title">
          <h1>
            <i className="fa fa-signal" aria-hidden="true" />
            {I18nUtils.t('att-price-page-title')}
...
ManagePriceRangerListPage.propTypes = {
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(ManagePriceRangerListPage))
