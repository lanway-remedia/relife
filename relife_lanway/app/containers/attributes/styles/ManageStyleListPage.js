...
class ManageStyleListPage extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      count: 0,
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.data != nextProps.data) {
      ...
      if (response.isDelete) {
        ...
        this.forceUpdate(this.getAttributeStyleList())
      }
    }
  }
  ...
  redirectToAddNew = () => {
    this.props.history.push('/add-new-house-style')
  }

  redirectToEdit = att => {
    this.props.history.push(`/edit-house-style/${att.id}`)
  }
  ...
  okFunction = att => {
    ...
    this.props.attributeStyleDeleteRequest(att.id)
    this.setState({
      title: att.title
    })
  ...
  render() {
    ...
    return (
      <Container fluid className="manage-attribute-list">
        <Helmet>
          <title>{I18nUtils.t('att-style-page-title')}</title>
        </Helmet>
        <div className="page-title">
          <h1>
            <i className="fa fa-signal" aria-hidden="true" />
            {I18nUtils.t('att-style-page-title')}
...
ManageStyleListPage.propTypes = {
  history: PropTypes.object,
  processing: PropTypes.bool,
  data: PropTypes.object,
  attributeStyleListRequest: PropTypes.func,
  attributeStyleDeleteRequest: PropTypes.func,
  show: PropTypes.func,
  hide: PropTypes.func
}
...
const mapDispatchToProps = dispatch => ({
  ...
  attributeStyleDeleteRequest: data =>
    dispatch(AttributeActions.attributeStyleDeleteRequest(data))
})

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(ManageStyleListPage))
