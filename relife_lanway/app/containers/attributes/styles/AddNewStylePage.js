...
class AddNewStylePage extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      data: [],
  ...
  handleSubmit = e => {
    ...
    this.props.attributeStyleAddRequest(data)
  }
  ...
  render() {
    return (
      <Container fluid className="add-new-attribute">
        <Helmet>
          <title>{I18nUtils.t('att-style-add-page-title')}</title>
        </Helmet>
        <div className="page-title">
          <h1>
            <i className="fa fa-signal" aria-hidden="true" />
            {I18nUtils.t('att-style-add-page-title')}
          </h1>
        </div>
        <ValidationForm
...
AddNewStylePage.propTypes = {
  history: PropTypes.object,
  attributeStyleAddRequest: PropTypes.func,
...
const mapDispatchToProps = dispatch => ({
  attributeStyleAddRequest: data =>
    dispatch(AttributeActions.attributeStyleAddRequest(data))
})

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(AddNewStylePage))
