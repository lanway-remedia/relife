...
class AddNewHouldHoldSizePage extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      data: [],
  ...
  handleSubmit = e => {
    ...
    this.props.attributeHouseSizeAddRequest(data)
  }
  ...
  render() {
    return (
      <Container fluid className="add-new-attribute">
        <Helmet>
          <title>{I18nUtils.t('att-hsize-add-page-title')}</title>
        </Helmet>
        <div className="page-title">
          <h1>
            <i className="fa fa-signal" aria-hidden="true" />
            {I18nUtils.t('att-hsize-add-page-title')}
          </h1>
        </div>
        <ValidationForm
...
AddNewHouldHoldSizePage.propTypes = {
  history: PropTypes.object,
  attributeHouseSizeAddRequest: PropTypes.func,
...
const mapDispatchToProps = dispatch => ({
  attributeHouseSizeAddRequest: data =>
    dispatch(AttributeActions.attributeHouseSizeAddRequest(data))
})

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(AddNewHouldHoldSizePage))
