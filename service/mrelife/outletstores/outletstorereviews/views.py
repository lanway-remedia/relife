...
from mrelife.outletstores.models import OutletStoreReview
...
class OutletStoreReviewViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk=None):
        try:
            ...
            queryset = OutletStoreReview.objects.all().filter(is_active=1).filter(is_active=1).order_by("-updated")
    ...
    def update(self, request, pk=None):
        ...
        try:
            ...
            return Response(CommonFuntion.resultResponse(False, "", MessageCode.OSC006.value, ""),  status=status.HTTP_404_NOT_FOUND)

    def destroy(self, request, pk=None):
        try:
            queryset = OutletStoreReview.objects.all()
            outletstoreObject = get_object_or_404(queryset, pk=pk)
